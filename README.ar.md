...
# XXD Panel 153｜خربشات يوميات أحادية اللون
...
تستخدم النماذج التالية مراجع أصلية مختلفة. أنشأ Panel 153 كل عمل بصورة مستقلة وفي جولة توليد واحدة، وقد أزيلت البيانات الوصفية للذكاء الاصطناعي. النماذج الأفقية تقسيمها صارم 50:50: الواقع يساراً والتصميم يميناً؛ والنماذج العمودية تقسيمها صارم 50:50: الواقع أعلى والتصميم أسفل.

حدود الأمثلة: تحتفظ بعض المناظر بتفاصيل وموضوعات أكبر من المطلوب، فلا تحقق الخربشات البسيطة القليلة والمحور بحجم طابع. أُبقيت نتائج التوليد الواحد لتوفير الحوسبة، وليست نماذج امتثال كامل؛ يظل التحقق أثناء التنفيذ صارماً.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 153 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-153`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-153 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-153-prompt.en.md) أو [بالصينية](references/xxd-panel-153-prompt.zh-CN.md).
